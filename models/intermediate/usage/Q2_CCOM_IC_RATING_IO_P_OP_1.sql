...
SELECT 
    ...
    REVENUE,
    COST
FROM {{ ref('Q2_CCOM_IC_RATING_IO_DUPL') }} MAIN
WHERE 1=1
   --AND FLAG_IND = 'Y'
   ...
   (
    ...
    FROM
        ...
        FROM {{ ref('Q2_CCOM_IC_RATING_IO_DUPL') }}
        WHERE 1=1
        --AND FLAG_IND = 'Y'
        ) A
    WHERE ABS(A.SUM_OUT_NBR_OF_UNITS - A.SUM_IN_NBR_OF_UNITS) <= 2 AND A.COUNT_OUT = 4
    GROUP BY 1,2
    )
)


--delete duplicate from pop_1
SELECT * FROM pop_1_raw_data
WHERE (OUT_CDR_FILE_ID, OUT_CDR_SERIAL_NR, COALESCE(OUT_NUMBER_OF_UNITS,0), COALESCE(OUT_NUMBER_OF_CALLS,0)) NOT IN
    (
