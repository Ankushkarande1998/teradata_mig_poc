...
SELECT 
    ...
    REVENUE,
    COST
FROM  {{ref('Q2_CCOM_IC_RATING_IO_DUPL_POP_2')}}
WHERE (OUT_CDR_FILE_ID, OUT_CDR_SERIAL_NR, COALESCE(OUT_NUMBER_OF_UNITS, 0)) IN (
    SELECT 
        A.OUT_CDR_FILE_ID, 
        A.OUT_CDR_SERIAL_NR, 
        COALESCE(A.OUT_NUMBER_OF_UNITS, 0)
    FROM (
        SELECT  
            OUT_CDR_FILE_ID,
            OUT_CDR_SERIAL_NR,
            COALESCE(OUT_NUMBER_OF_UNITS, 0) AS OUT_NUMBER_OF_UNITS,
            SUM(COALESCE(IN_NUMBER_OF_UNITS, 0)) 
                OVER (PARTITION BY OUT_CDR_FILE_ID, OUT_CDR_SERIAL_NR) 
                AS SUM_IN_NBR_OF_UNITS
        FROM {{ref('Q2_CCOM_IC_RATING_IO_DUPL_POP_2')}}
        WHERE 1=1
    ) A
    WHERE ABS(A.OUT_NUMBER_OF_UNITS - A.SUM_IN_NBR_OF_UNITS) <= 2
    GROUP BY A.OUT_CDR_FILE_ID, A.OUT_CDR_SERIAL_NR, A.OUT_NUMBER_OF_UNITS
)
)

--delete duplicate from pop_1
SELECT * FROM pop_3_raw_data
